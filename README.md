...
- 04_重建二叉树
    - 前序 + 中序 ->推断：二叉树
    - 前序中找到root，中序中分辨左、右子树；递归左、右子树，获得二叉树
- 17_树的子结构
    - 1. 在树A中找到和树B的根结点值一样的结点R
    - 2. 判断树A中以R为根结点的子树是不是包含和树B一样的结构
- 18_二叉树的镜像
    - 考察二叉树的遍历方式：BFS、DFS都可以，遍历到节点时，交换左右子树
    - 递归也可以
- 22_从上往下打印二叉树 BFS
    - 层次遍历：BFS
    - 小技巧：一次大循环中，queue.size表示当前层级的节点个数
- 23_二叉搜索树的后序遍历序
    - 要知道BST是什么？
    - 递归：最后一个根节点；遍历，小于根节点的是左子树；大于根节点的是右子树；递归比较左右子树
    - TODO：其实还是有点懵逼的
- 24_二叉树中和为某一值的路径
    - 递归：注意结束条件(找到叶子节点)；要回溯；左子树、右子树分别递归
- 26_二叉搜索树与双向链表
    - TODO:（有点不太熟练）遍历两次 -> 遍历一次，但是需要额外root节点 -> 遍历一次，不需要root节点
- 38_二叉树的深度
    - 递归法：树的深度为左、右子树深度的较大值加1
    - 层次遍历：未poll前的queue.size表示当前层总的节点数
- 39_平衡二叉树
    - 什么是平衡二叉树？ 
        - 它的左右两个子树的高度差的绝对值不超过1，&& 左右两个子树都是一棵平衡二叉树
        - 空树
    - 递归法：注意用map，减少重复计算子树高度的次数
    - 自底向上：利用后序遍历：左子树、右子树、根节点,可以先递归到叶子节点，然后在回溯的过程中来判断是否满足条件
- 57_二叉树的下一个结点
    - 直接找下一结点：(需要分析特点，归为几个类别)
        - 有右子树
        - 有左子树：并且是父节点的左结点
        - 有左子树：并且是父节点的右结点
    - 还原二叉树
- 58_对称的二叉树
    - 递归法：左的左比较右的右，左的右比较右的左
    - BFS法：不易想到，顺序如上，根据比较对象决定插入队列的顺序，只是递归换成queue
- 59_按之字形顺序打印二叉树
    - 两个栈
    - BFS：一个queue，一个标志位isReverse翻转
- 60_把二叉树打印成多行
    - BFS：注意换行的关键：未poll前的queue.size表示当前层总的节点数
- 61_序列化二叉树
    - 序列化：先序遍历，"#"表示叶子节点（正常 先序+中序 可以推断二叉树，这里有#，可以不用中序）
    - 反序列化：先序遍历，"#"返回null
- 62_二叉搜索树的第k个结点
    - 二叉搜索树中序遍历正好就是递增序列
    - 递归法：中序遍历
...
2. 二叉搜索树 BST
    ...
    - ex: 
        ...
        - 二叉搜索树的第k个结点
3. 平衡二叉树
    - 它的左右两个子树的高度差的绝对值不超过1；空树
4. 二叉树的推断：
    - 前序 + 中序 -> 二叉树(重建二叉树)
    - 前序 + # -> 二叉树(序列化二叉树)
5. 表示当前层总的节点数:queue.size
    - ex
        - 从上往下打印二叉树
        - 二叉树的深度
        - 把二叉树打印成多行
