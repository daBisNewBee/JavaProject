...
//    System.getProperties().put("sun.misc.ProxyGenerator.saveGeneratedFiles","true");
    ...
    String proxyName = "PopProxy";
    byte[] proxyClassFile = ProxyGenerator.generateProxyClass(proxyName, pop.getClass().getInterfaces());
    File f = new File("PopProxy.class");
    ...
    /*
    此处动态代理的目的：
    为了生成"OkHttpCall"的实例，将注解参数构造成http请求，实际的执行还是由okhttp完成。
    区别于常规的DP，需要注入实例对象到"InvocationHandler"，并在调用方法时回调invoke
    */
    Call<ResponseBody> call = pop.robots();
    Response<ResponseBody> response1 = call.execute();
